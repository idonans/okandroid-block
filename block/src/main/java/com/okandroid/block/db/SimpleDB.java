package com.okandroid.block.db;
...
public class SimpleDB {
  ...
  public SimpleDB(@NonNull String databaseName) {
    ...
    mOpenHelper = new SQLiteOpenHelper(ContextUtil.getContext(), dbName, null, DB_VERSION) {
  ...
  @CheckResult public String get(@Nullable String key) {
    ...
    try {
      ...
      cursor = db.query(TABLE_NAME, new String[] { COLUMN_VALUE }, COLUMN_KEY + "=?",
  ...
  public int trim(int maxRows) {
    ...
    try {
      ...
      long lastUpdate = 0L;
      try {
        cursor = db.query(TABLE_NAME, new String[] { COLUMN_UPDATE }, null, null, null, null,
            COLUMN_UPDATE + " desc", (maxRows - 1) + ",1");
        if (cursor.moveToFirst()) {
      ...
      return db.delete(TABLE_NAME, COLUMN_UPDATE + "<?",
          new String[] { String.valueOf(lastUpdate) });
    } catch (Throwable e) {
      e.printStackTrace();
    }
    return -1;
  ...
  public void printAllRows() {
    ...
    try {
      ...
      Log.d("--" + tag + "--");
      cursor =
          db.query(TABLE_NAME, new String[] { COLUMN_KEY, COLUMN_VALUE, COLUMN_UPDATE }, null, null,
              null, null, COLUMN_UPDATE + " desc");
      String key;
      String value;
      long update;
