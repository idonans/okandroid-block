 ...
 * 提供 k-v 数据存储, 支持跨进程.
 *
 * 提供了 setting 和 cache 两个不同的存储空间，各自独立运行
 */
public class StorageDatabaseProvider extends ContentProvider {
  ...
  public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection,
    ...
    if (selectionArgs == null || selectionArgs.length != 1) {
      new IllegalArgumentException(
          "error selection args " + Arrays.deepToString(selectionArgs)).printStackTrace();
      return null;
    }
  ...
  @Nullable private SimpleDB getTargetDatabase(Uri uri) {
    switch (mUriMatcher.match(uri)) {
      ...
      case URI_SETTING:
        return mDBSetting;
      default:
        new IllegalArgumentException("target database not found for uri " + uri).printStackTrace();
        return null;
    }
  }
}
