...
public class SystemUtil {
   ...
   * 容量超过此值会出现 OOM 错误.
   * 如 手机 CHM-UL00 的 heap size 是 268435456 byte (256M), 该手机的配置是 2G 内存，16G 存储空间, 1280x720 分辨率,
   * Android 4.4.2 系统
   */
  ...
  public static boolean showSoftKeyboard(EditText editText) {
    InputMethodManager inputMethodManager = (InputMethodManager) ContextUtil.getContext()
  ...
  private static String getPadding(View view) {
    return String.format(Locale.getDefault(), "[%s, %s, %s, %s]", view.getPaddingLeft(),
