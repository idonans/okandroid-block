...
public class ImageCacheUtil {
                                ...
                                try {
                                    ...
                                    File targetFile =
                                            TmpFileManager.getInstance()
                                                    .createNewTmpFileQuietly(
                                                            CACHE_IMAGE_PREFIX, extension);
                                    if (targetFile == null) {
                                        throw new IllegalArgumentException(
                                                "fail to create target file");
                                    }
     ...
     */
    public static void cacheImageThumb(
            final String imageUrl,
            final int width,
            final int height,
            final long maxFileSize,
            final ImageCacheListener listener) {
        // init fresco if need.
        FrescoManager.getInstance();
                                    ...
                                    if (bitmap.compress(Bitmap.CompressFormat.JPEG, 85, fos)) {
                                        ...
                                        if (fileLength <= 0) {
                                            ...
                                            throw new IllegalAccessException(
                                                    "invalid file length " + fileLength);
                                            ...
                                            // guess scale size
                                            float scaleSize =
                                                    (Math.max(minFileLength, maxFileSize))
                                                            * 1f
                                                            / fileLength
                                                            * 1.5f;
