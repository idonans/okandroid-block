...
public class ImageCacheUtil {
        ...
        .subscribe(new BaseDataSubscriber<CloseableReference<PooledByteBuffer>>() {
          @Override protected void onNewResultImpl(
            ...
            try {
              ...
              File targetFile = TmpFileManager.getInstance()
                  .createNewTmpFileQuietly(CACHE_IMAGE_PREFIX, extension);
              if (targetFile == null) {
   ...
   */
  public static void cacheImageThumb(final String imageUrl, final int width, final int height,
      final long maxFileSize, final ImageCacheListener listener) {
    // init fresco if need.
    FrescoManager.getInstance();
    ...
    builder.setImageDecodeOptions(ImageDecodeOptions.newBuilder()
    ...
    Fresco.getImagePipeline()
        ...
        .subscribe(new BaseBitmapDataSubscriber() {
          @Override protected void onNewResultImpl(Bitmap bitmap) {
            ...
            try {
              File targetFile = TmpFileManager.getInstance()
                  .createNewTmpFileQuietly(CACHE_IMAGE_THUMB_PREFIX, ".jpg");
              if (targetFile == null) {
                throw new IllegalArgumentException("fail to create target file");
              }
              ...
              if (bitmap.compress(Bitmap.CompressFormat.JPEG, 85, fos)) {
                ...
                if (fileLength <= 0) {
                  ...
                  Log.v(TAG, "file size too large, scale size and try again", "fileLength:",
                      HumanUtil.getHumanSizeFromByte(fileLength), "maxFileSize:",
                      HumanUtil.getHumanSizeFromByte(maxFileSize), "minFileLength:",
                      HumanUtil.getHumanSizeFromByte(minFileLength), "width:", resizeWidth,
                      "height:", resizeHeight, "scaleSize:", scaleSize);
                  cacheImageThumb(imageUrl, (int) (resizeWidth * scaleSize),
                      (int) (resizeHeight * scaleSize), maxFileSize, listener);
                }
              }
            } catch (Throwable e) {
              e.printStackTrace();
  ...
  private static class OnceImageCacheListener implements ImageCacheListener {
    ...
    @Override public void onImageCached(@Nullable File file) {
